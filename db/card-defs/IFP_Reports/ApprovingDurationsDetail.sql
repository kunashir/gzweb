...
EXEC [dbo].[dvreport_get_data_{EB5F2448-894F-4FA7-B20D-DCAE1E1A1548}] @startDate, @endDate
...
IF @CardTypeId = '{2E729D50-6B61-4186-9D14-44E109FD920B}'
	BEGIN
		...
		MainInfo.State,
		(SELECT TOP(1) StartsAt FROM @generalReportData AS repData WHERE repData.InstanceID = MainInfo.InstanceID AND repData.RangeType = 0) as ApprovingStartsAt,
		(SELECT TOP(1) EndsAt FROM @generalReportData AS repData WHERE repData.InstanceID = MainInfo.InstanceID AND repData.RangeType = 0) as ApprovingEndsAt,
		(SELECT TOP(1) StartsAt FROM @generalReportData AS repData WHERE repData.InstanceID = MainInfo.InstanceID AND repData.RangeType = 1) as SignPrepareStartsAt,
		(SELECT TOP(1) EndsAt FROM @generalReportData AS repData WHERE repData.InstanceID = MainInfo.InstanceID AND repData.RangeType = 1) as SignPrepareEndsAt,
		(SELECT TOP(1) StartsAt FROM @generalReportData AS repData WHERE repData.InstanceID = MainInfo.InstanceID AND repData.RangeType = 2) as SignStartsAt,
		(SELECT TOP(1) EndsAt FROM @generalReportData AS repData WHERE repData.InstanceID = MainInfo.InstanceID AND repData.RangeType = 2) as SignEndsAt
		FROM
		...
		WHERE EXISTS (SELECT * FROM @generalReportData AS Data WHERE Data.InstanceID = MainInfo.InstanceID)
		AND Employees.RowID = MainInfo.Executer AND Units.RowID = Employees.ParentRowID
		AND (@byDepartment = 0 OR (Units.Comments is null AND IsNULL(@depName,'') = '') OR Convert(nvarchar(1024), Units.Comments) = @depName)
...
--CONTRACT
	ELSE IF @CardTypeId = '{F8EE21FE-FB3D-45AD-B2C9-10C5A224E2D9}'
		BEGIN
			...
			EXISTS (SELECT * FROM @generalReportData AS Data WHERE Data.InstanceID = MainInfo.InstanceID)
