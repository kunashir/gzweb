	...
	<xsl:template name="printresolution">
		...
		<tr>
			...
			<td class="fieldheader" style="border-top-style: solid; border-top-color: #CCCCCC; border-top-width: 1px;">
				...
				<xsl:if test="string-length(CardReport/MainInfo/@DocType_Name)=0">Resolution</xsl:if>
			</td>
		...
		<tr>
			...
			<td>
				<xsl:choose>
					<xsl:when test="$resolution/CardResolution/MainInfo/@ResolutionState=-1">Error</xsl:when>
					<xsl:when test="$resolution/CardResolution/MainInfo/@ResolutionState=0">Non active</xsl:when>
					<xsl:when test="$resolution/CardResolution/MainInfo/@ResolutionState=1">To perform</xsl:when>
					<xsl:when test="$resolution/CardResolution/MainInfo/@ResolutionState=2">In process</xsl:when>
					<xsl:when test="$resolution/CardResolution/MainInfo/@ResolutionState=3">Completed</xsl:when>
					<xsl:when test="$resolution/CardResolution/MainInfo/@ResolutionState=4">Recalled</xsl:when>
				</xsl:choose>
			</td>
		</tr>
		...
		<xsl:for-each select="$resolution/Tasks/Data/Reports/Data">
			<tr>
				<td></td>
				<td class="fieldheader">Report</td>
				<td colspan="5">
			...
			<tr>
				...
				<td>
					<xsl:choose>
						<xsl:when test="CardReport/MainInfo/@ReportState=-1">Error</xsl:when>
						<xsl:when test="CardReport/MainInfo/@ReportState=0">Non active</xsl:when>
						<xsl:when test="CardReport/MainInfo/@ReportState=1">To control</xsl:when>
						<xsl:when test="CardReport/MainInfo/@ReportState=2">On control</xsl:when>
						<xsl:when test="CardReport/MainInfo/@ReportState=3">Accepted</xsl:when>
						<xsl:when test="CardReport/MainInfo/@ReportState=4">Rejected</xsl:when>
					</xsl:choose>
				</td>
			</tr>
			<tr>
		...
		<html>
			...
			<body>
				...
				<table>
					...
					<th class="tableheader">Performers</th>
					<th class="tableheader">Performing date</th>
					<th class="tableheader">Control date</th>
					<th class="tableheader">Controller</th>
				...
				<p class="smalltext">
					In order to change the printing template, edit Print Template for the Resolution tree or create a new one based on this template.
